...
class Lib_common extends CI_Controller {
    ...
    function isValidUser($create_by){
      ...
      $info = $this->CI->common_model->get_entry_by_data('users',true,array('create_by' => $create_by ),'create_by');
      if(!empty($info))
        return true;
    ...
    function isValidAPIKey($create_by, $api_key){
      ...
      $info = $this->CI->common_model->get_entry_by_data('users',true,array( 'create_by' => $create_by, 'api_key' => $api_key ),'create_by');
